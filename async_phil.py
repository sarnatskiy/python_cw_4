...
async def build_node(session, frontier):
    tasks = [get_content(session, name) for name in frontier]
...
#     return graph
async def build_graph(start, finish):
    graph = defaultdict(set)
    visited = set()
    frontier = [start]
    cf_finish = finish.casefold()
    async with aiohttp.ClientSession() as session:
        while cf_finish not in (visited | set(frontier)):
            new_front = []
            async for (name, link) in build_node(session, frontier):
                visited.add(name.casefold())
                graph[name].add(link)
                if link.casefold() not in visited:
                    new_front.append(link)
...
async def amain():
    if len(sys.argv) < 2:
        sys.exit("Start word is not specified")
    params = (sys.argv[1], 'Философия')
    graph = await build_graph(*params)
    chain = find_chain(graph, *params)
    if chain:
        print('\n'.join(chain))
    else:
